...
// ⚠️ On n'applique pas express.json() sur TOUTES les requêtes.
// On l’appliquera seulement sur celles qui ne sont pas "multipart/form-data".
app.use((req, res, next) => {
  const contentType = req.headers["content-type"] || "";
  if (contentType.startsWith("multipart/form-data")) {
    // On laisse multer gérer plus tard
    return next();
  }
...
(async () => {
  ...
  // ⚠️ Tes routes (dont l’upload) doivent être enregistrées ici
